...
`TaskDataStore` is a task-scoped, in-memory store that allows plugins to share data during the lifecycle of a task. Use it when the data only matters while a task is active and should not persist once the task is complete.

### Stores and behavior

TaskDataStore has two separate stores:
...
## Directory metadata (.unmanic) (deprecated)
...
:::warning
This feature is deprecated in favor of the database-backed `UnmanicFileMetadata` helper class described below, and `.unmanic` support may be removed in a future Unmanic update.
:::
...
`UnmanicFileMetadata` stores file-level metadata that persists across tasks and survives renames or moves by using a content fingerprint.
...
By default, `set()` writes metadata that will be committed against the source fingerprint. If your plugin needs to persist metadata for the destination file(s) on success, pass `use_destination_scope=True`. A plugin can write both scopes by calling `set()` twice with different `use_destination_scope` values.
...
`set(data, use_destination_scope=False)`

- **`data`** _[dictionary]_ - Data to merge into your plugin’s metadata namespace.
- **`use_destination_scope`** _[boolean]_ - When `True`, commit metadata against destination fingerprints on success. Default `False` commits against the original source fingerprint.
...
- When `use_destination_scope=False` (default), the source file fingerprint is captured at the first call to `set()`; if the source file no longer exists at commit time, the metadata is dropped and an info log is written.
...
- Use `TaskDataStore` when the data only matters during a task and should not persist afterward.
- Use `UnmanicFileMetadata` when you need to persist file-level decisions across scans, renames, or moves.
