...
## Optional Helper Kwargs

Some runner types can opt into helper classes that Unmanic injects by keyword. Only declare the helpers your runner actually uses.
...
- `task_data_store` provides task-scoped shared state between runners and plugins.
- `file_metadata` provides persistent file-scoped metadata lookups and writes.
- See [Sharing Data Between Plugins](/docs/development/writing_plugins/sharing_data_between_plugins) for usage patterns and API details.
...
If your file test runner needs persistent file-scoped decisions from earlier tasks, it may also declare:
...
Use `file_metadata` here for read-only lookups by path. See [Sharing Data Between Plugins](/docs/development/writing_plugins/sharing_data_between_plugins#file-metadata-helper).
...
The `shared_info` dictionary allows plugins to share data with subsequent plugins in the same flow. This is particularly useful for caching expensive operations like ffprobe so multiple plugins don't repeat the same work.

**How it works:**

1. The first plugin in the flow receives an empty `shared_info` dict
2. That plugin can store data in `shared_info` (e.g., ffprobe results)
3. Subsequent plugins receive the same `shared_info` with the stored data
4. Each plugin can read existing data and/or add more

**Common pattern - caching ffprobe data:**
...
- Running ffprobe is relatively slow (disk I/O + process spawn)
- A library with 5 file test plugins would normally probe each file 5 times
- With shared_info caching, the file is probed once and results are reused
- This significantly speeds up library scans on large collections

**Best practices:**

- Always check if data exists in `shared_info` before doing expensive operations
- Use descriptive keys (e.g., `'ffprobe'`, `'mediainfo'`) to avoid collisions
- Only store serializable data (dicts, lists, strings, numbers)
- Keep stored data minimal - don't cache entire file contents
...
If your worker runner needs shared task state or persistent file metadata, declare the helpers it uses:
...
See [Sharing Data Between Plugins](/docs/development/writing_plugins/sharing_data_between_plugins).
...
- **`library_id`** _[integer]_ - The ID of the library that the current task is associated with.
- **`exec_command`** _[list]_ - A subprocess command that Unmanic should execute. Can be left empty.
...
`exec_command` should be returned as an argv-style list. Older plugins may still return a string for compatibility,
but this is deprecated and may still be executed via a shell during the transition period.
If your plugin needs shell features such as pipes, redirects, or multi-step shell logic, prefer running an explicit
wrapper script or use `PluginChildProcess` rather than relying on this compatibility behaviour long-term.
:::
...
def child_work(source_path, log_queue=None, prog_queue=None):
    # PluginChildProcess injects log_queue and prog_queue as keyword arguments.
    # Any positional arguments should be passed to proc.run(...) first.
    log_queue.put(f"Starting repair work for {source_path}")
...
def on_worker_process(data):
    ...
    # "data" still carries the worker state, including worker_log/current_command.
    success = proc.run(child_work, data["file_in"])
    if success:
        repaired_cache_path = data["file_out"]
        # Child-process plugins should publish the new cache artifact as both
        # file_out and file_in when later worker plugins in the chain need to
        # consume it.
...
2. Calls your target with `target(*args, **kwargs)` after injecting `log_queue` and
   `prog_queue` into the keyword arguments.
3. Registers its PID & start time with the worker’s `default_progress_parser`.
4. Drains `log_queue` → appends to `data["worker_log"]` for UI tail.
5. Drains `prog_queue` → passes to `command_progress_parser(line_text)` to update the progress bar.
6. Automatically unsets the child process PID on exit to reset all tracked subprocess metrics
   in the Unmanic Worker (CPU, memory, progress, etc.).

When using `PluginChildProcess`, Unmanic does not automatically promote a newly
created `file_out` into `file_in` for later worker plugins in the chain. If
your child process creates a new cache artifact that subsequent worker plugins
should process, update both:

- `data["file_out"]` to the new cache artifact path
- `data["file_in"]` to that same path

This keeps the worker chain aligned with the file your plugin actually produced.
...
If your postprocessor needs task-scoped shared state or source/destination file metadata, declare the helper kwargs it uses:
...
If your task-result runner needs task-scoped shared state or persistent file metadata, declare the helper kwargs it uses:
...
Frontend panel runners usually only need `data`. Only declare helper kwargs if you have a concrete need for them, and prefer the patterns documented in [Sharing Data Between Plugins](/docs/development/writing_plugins/sharing_data_between_plugins) for task/file state rather than inventing your own storage.
...
Plugin API runners usually only need `data`. Only declare helper kwargs if you have a concrete need for them, and prefer the patterns documented in [Sharing Data Between Plugins](/docs/development/writing_plugins/sharing_data_between_plugins) for task/file state rather than inventing your own storage.
...
- **`file_move_processes_success`** _[boolean]_ - True if all postprocessor movement tasks completed successfully.
- **`start_time`** _[float]_ - UNIX timestamp when the task began.
- **`finish_time`** _[float]_ - UNIX timestamp when the task completed.
