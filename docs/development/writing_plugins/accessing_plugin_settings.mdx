...
#### _classmethod_ **get_plugin_directory()**
...
#### _classmethod_ **get_profile_directory()**
...
#### _classmethod_ **get_form_settings()**
...
:::caution
If no _library_id_ is provided to the Settings object, then it will return the global settings.
If no config has been saved for the given _library_id_, this method will fallback to returning the global settings.
See example below.
:::
...
#### _classmethod_ **set_setting(** _key_, _value_ **)**
...
:::caution
If no _library_id_ is provided to the Settings object, then it will be stored in the global settings file.
If no config has been saved for the given _library_id_, this method will create that library's config file.
See example below.
:::
...
<img
  ...
  src={require("/img/unmanic-plugin-settings-basic.png").default}
/>
...
- [Mark sub-settings](#sub-settings)
...
<img
  className={"screenshot"}
  src={
    require("/img/unmanic-plugin-settings-advanced-dynamic-hidden1.png").default
  }
/>
<img
  className={"screenshot"}
  src={
    require("/img/unmanic-plugin-settings-advanced-dynamic-hidden2.png").default
  }
/>
...
If you want a setting to appear visually grouped under another option (for example, a dependent option that only makes sense when a checkbox is enabled), you can mark it as a sub-setting.

Add `sub_setting: True` to that setting's form configuration to indent it and draw a left border in the WebUI.
...
### Admonitions and Informational Sections

You can render non-input UI elements inside plugin settings forms by using one of the section-style `input_type` values.
These are useful when a plugin needs to explain a warning, group related settings, or show contextual notes without storing additional user input.

Available section input types include:
...
Use `section_admonition` when you need a visible warning, note, or tip banner in the settings UI.
The banner type is derived from the `label`, so values such as `Warning`, `Info`, or `Note` are appropriate.
The `description` value supports HTML and is rendered in the WebUI.
...
class Settings(PluginSettings):
    ...
    form_settings = {
        ...
        "audio_codec_admonition": {
            ...
            "description": (
                "Selected codec supports up to <code>2</code> channels.<br>"
                "Using MP3 on multichannel movie audio will downmix the output stream."
            ),
        },
    }
```

If you only want the admonition to appear for certain states, combine it with the normal dynamic display behavior:

```python
def __set_audio_codec_admonition_form_settings(self):
    values = {
        "label":      "Warning",
        "input_type": "section_admonition",
        "description": "This codec is limited to stereo output.",
    }
    if self.get_setting('audio_codec') != 'mp3':
        values["display"] = 'hidden'
    return values
```
...
<img
  className={"screenshot"}
  src={require("/img/unmanic-plugin-settings-advanced-labels.png").default}
/>
...
<img
  className={"screenshot"}
  src={require("/img/unmanic-plugin-settings-advanced-select.png").default}
/>
...
<img
  className={"screenshot"}
  src={require("/img/unmanic-plugin-settings-advanced-textarea.png").default}
/>
...
<img
  className={"screenshot"}
  src={
    require("/img/unmanic-plugin-settings-advanced-browse-directory.png")
      .default
  }
/>
...
<img
  className={"screenshot"}
  src={require("/img/unmanic-plugin-settings-advanced-slider.png").default}
/>
