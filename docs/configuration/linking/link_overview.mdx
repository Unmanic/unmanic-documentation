...
:::tip
You can still use "Automatically configure missing libraries on the remote installation" if you want a one-time push of the local library configuration to a new remote installation, then edit plugin settings independently on each install.

If you also enable "Keep remote library and plugin configuration up to date for every sent file", Unmanic will re-sync that matching remote library before each file transfer so both installations continue using the same library and plugin configuration. See [Remote Installation Configuration](./link_remote_configuration).
:::
...
- The auto-configured library is configured as remote-only with scanning disabled.
- The task is still first attempted by resolving the relative file path under the remote library path.
- If the remote installation can resolve that path on shared storage, the task is created and processed directly from that path.
- If the remote installation cannot resolve the path, Unmanic falls back to the HTTP upload/download flow.

Example 4: Auto-configure plus keep-updated enabled

- Local `Media` library queues a task.
- Link manager finds a matching `Media` library on the remote installation.
- Before the file is sent, Unmanic re-imports the local `Media` library configuration and plugin stack onto the remote `Media` library.
- The task is then sent and processed using the refreshed remote library configuration.

Example 5: Different path strings, same shared storage
